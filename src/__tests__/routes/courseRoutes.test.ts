...
describe("Course Routes", () => {
  ...
  const setupMockAuth = () => {
    ...
    mockRequireAdmin.mockImplementation(
      (req: Request, res: Response, next: NextFunction) => {
        ...
        const error = new (AppError as jest.MockedClass<typeof AppError>)(
          403,
    ...
    (AppError as jest.MockedClass<typeof AppError>).mockImplementation(
  ...
  beforeEach(() => {
    ...
    mockCourseController =
      new CourseController() as jest.Mocked<CourseController>;
    ...
    mockCourseController.getPublishedCourses = jest
      .fn()
      .mockImplementation((req, res) => {
        res.status(200).json([{ id: 1, title: "Published Course 1" }]);
      });
    mockCourseController.getCourseWithModules = jest
      .fn()
      .mockImplementation((req, res) => {
        res
          .status(200)
          .json({ id: req.params.id, title: "Test Course", modules: [] });
      });
    mockCourseController.togglePublish = jest
      .fn()
      .mockImplementation((req, res) => {
    ...
    (asyncHandler as jest.Mock).mockImplementation(
    ...
    (CourseController as jest.Mock).mockImplementation(
      () => mockCourseController,
    );
    ...
    testRouter.post(
      ...
      asyncHandler((req, res) => mockCourseController.create(req, res)),
    );
    // Assuming GET routes might require authentication but not admin rights
    testRouter.get(
      "/",
      mockAuthenticate,
      asyncHandler((req, res) => mockCourseController.findAll(req, res)),
    );
    testRouter.get(
      "/published",
      mockAuthenticate,
      asyncHandler((req, res) =>
        mockCourseController.getPublishedCourses(req, res),
      ),
    );
    testRouter.get(
      "/:id",
      mockAuthenticate,
      asyncHandler((req, res) => mockCourseController.findOne(req, res)),
    );
    testRouter.get(
      "/:id/with-modules",
      mockAuthenticate,
      asyncHandler((req, res) =>
        mockCourseController.getCourseWithModules(req, res),
      ),
    );
    ...
    testRouter.put(
      ...
      asyncHandler((req, res) => mockCourseController.update(req, res)),
    );
    testRouter.delete(
      "/:id",
      mockAuthenticate,
      mockRequireAdmin,
      asyncHandler((req, res) => mockCourseController.delete(req, res)),
    );
    testRouter.patch(
      "/:id/toggle-publish",
      mockAuthenticate,
      mockRequireAdmin,
      asyncHandler((req, res) => mockCourseController.togglePublish(req, res)),
    );
  ...
  describe("POST /api/courses", () => {
    ...
    it("should return 401 Unauthorized if no token is provided", async () => {
      await request(app).post("/api/courses").send(courseData).expect(401);
  ...
  describe("GET /api/courses/published", () => {
    it("should return 401 Unauthorized if no token is provided", async () => {
      await request(app).get("/api/courses/published").expect(401);
  ...
  describe("GET /api/courses/:id", () => {
    it("should return 401 Unauthorized if no token is provided", async () => {
      await request(app).get("/api/courses/1").expect(401);
  ...
  describe("GET /api/courses/:id/with-modules", () => {
    it("should return 401 Unauthorized if no token is provided", async () => {
      await request(app).get("/api/courses/1/with-modules").expect(401);
    ...
    it("should get a course with its modules (200) if user is authenticated", async () => {
      ...
      expect(mockCourseController.getCourseWithModules).toHaveBeenCalledTimes(
        1,
      );
    });
  });
  ...
  describe("PUT /api/courses/:id", () => {
    ...
    it("should return 401 Unauthorized if no token is provided", async () => {
      await request(app).put("/api/courses/1").send(courseData).expect(401);
  ...
  describe("DELETE /api/courses/:id", () => {
    it("should return 401 Unauthorized if no token is provided", async () => {
      await request(app).delete("/api/courses/1").expect(401);
  ...
  describe("PATCH /api/courses/:id/toggle-publish", () => {
    it("should return 401 Unauthorized if no token is provided", async () => {
      await request(app).patch("/api/courses/1/toggle-publish").expect(401);
