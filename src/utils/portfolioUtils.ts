...
export function calculateTotals(balances: AssetBalance[]): {
  ...
  const totalCost = balances.reduce((sum, balance) => {
    return sum + balance.total * balance.averageCost
  }, 0)
...
export function calculateAllocation(balances: AssetBalance[]): Array<{
  ...
  return balances
    ...
    .map((balance, index) => ({
      ...
      color: assetColors[balance.asset] || `hsl(${(index * 137.5) % 360}, 70%, 50%)`,
    }))
...
export function isPortfolioEmpty(balances: AssetBalance[]): boolean {
  return !balances || balances.length === 0 || balances.every((balance) => balance.total === 0)
}
...
export function getPerformanceMetrics(balances: AssetBalance[]): {
  ...
  for (const balance of assetsWithHoldings) {
    ...
    if (!bestPerformer || balance.unrealizedPnLPercentage > bestPerformer.unrealizedPnLPercentage) {
      bestPerformer = balance
    }

    if (!worstPerformer || balance.unrealizedPnLPercentage < worstPerformer.unrealizedPnLPercentage) {
