...
export function calculateTotals(balances: NumericBalance[], prices: Record<string, number>): PortfolioTotals {
...
export function calculateAssetPnL(balance: NumericBalance, currentPrice: number): AssetPnL {
  const total = new Decimal(balance.total)
...
export function calculateFIFOCostBasis(transactions: NumericTransaction[]): CostBasisResult {
  ...
  for (const tx of transactions) {
    ...
    if (tx.type === "BUY" || tx.type === "DEPOSIT" || tx.type === "TRANSFER_IN" || tx.type === "REWARD") {
    ...
    } else if (tx.type === "SELL" || tx.type === "WITHDRAWAL" || tx.type === "TRANSFER_OUT") {
  ...
  const averageCost = remainingQuantity.greaterThan(0) ? totalCost.dividedBy(remainingQuantity).toNumber() : 0
...
export function calculatePerformanceMetrics(
  ...
  const percentageChange = previous.greaterThan(0) ? absoluteChange.dividedBy(previous).times(100) : new Decimal(0)
...
export function calculateSharpeRatio(returns: number[], riskFreeRate = 0.02): number {
  ...
  const mean = returnsDecimal.reduce((sum, r) => sum.plus(r), new Decimal(0)).dividedBy(returns.length)
...
export function calculateDiversificationScore(allocation: AllocationItem[]): number {
  ...
  const diversificationScore = new Decimal(1).minus(hhi.minus(minHHI).dividedBy(maxHHI.minus(minHHI))).times(100)
