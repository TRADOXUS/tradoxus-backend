...
export class PortfolioController {
  ...
  getPortfolioSummary: RequestHandler = asyncHandler(
  ...
  getAssetBalances: RequestHandler = asyncHandler(
    async (req: Request, res: Response) => {
      const userId = req.user?.id;
      if (!userId) {
        throw new AppError("User not authenticated", 401);
  ...
  getTransactionHistory: RequestHandler = asyncHandler(
    async (req: Request, res: Response) => {
      const userId = req.user?.id;
      if (!userId) {
        throw new AppError("User not authenticated", 401);
      ...
      const result = await this.portfolioService.getTransactionHistory(
        userId,
        limit,
        offset,
        asset,
  ...
  getPortfolioPerformance: RequestHandler = asyncHandler(
    async (req: Request, res: Response) => {
      const userId = req.user?.id;
      if (!userId) {
        throw new AppError("User not authenticated", 401);
  ...
  getPortfolioHistory: RequestHandler = asyncHandler(
    async (req: Request, res: Response) => {
      const userId = req.user?.id;
      if (!userId) {
        throw new AppError("User not authenticated", 401);
  ...
  static validateTransactionHistory: RequestHandler[] = [
    ...
    query("asset")
      ...
      .withMessage("Asset must be 1–20 characters"),
    validateRequestEx,
  ];
  ...
  static validateHistoryQuery: RequestHandler[] = [
    ...
    query("points")
      .optional()
      .isInt({ min: 1, max: 365 })
      .withMessage("Points must be between 1 and 365"),
    validateRequestEx,
  ];
}
