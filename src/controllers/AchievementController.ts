...
export class AchievementController {
  ...
  async findByModuleId(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!moduleId) {
        return res
          .status(400)
          .json({ status: "error", message: "Module ID is required" });
      }

      const achievements =
        await this.achievementService.findByModuleId(moduleId);
  ...
  async claimAchievement(req: Request, res: Response): Promise<void> {
    try {
      const achievement = await this.achievementService.claimAchievement(
        req.params.userId,
  ...
  async getUserProgress(req: Request, res: Response): Promise<void> {
    try {
      const progress = await this.achievementService.getUserProgress(
        req.params.userId,
      );
  ...
  async updateProgress(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const updatedProgress = await this.achievementService.updateProgress(
        userId,
        achievementId,
        progress,
      );
      ...
      if (!updatedProgress) {
        return res
          .status(404)
          .json({ status: "error", message: "User achievement not found" });
      }
