...
export class ModuleController {
    ...
    async createInCourse(req: Request, res: Response): Promise<void> {
        try {
            ...
            const module = await this.moduleService.createInCourse(courseId, req.body);
            res.status(201).json({
                status: 'success',
                data: module
            });
    ...
    async getModulesByCourse(req: Request, res: Response): Promise<void> {
        try {
            ...
            const page = parseInt(req.query.page as string) || 1;
            const limit = parseInt(req.query.limit as string) || 10;
            const { items, total } = await this.moduleService.getModulesByCourse(courseId, page, limit);
    ...
    async reorderModules(req: Request, res: Response): Promise<void> {
        try {
            ...
            const success = await this.moduleService.reorderModules(courseId, moduleOrders);
            if (!success) {
