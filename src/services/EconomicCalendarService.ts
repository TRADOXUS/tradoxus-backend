...
import { CalendarQueryDto, CreateAlertDto } from "../dto/EconomicCalendarDto";
...
export class EconomicCalendarService {
  ...
  constructor() {
    ...
    this.eventImpactAnalysisRepository =
      AppDataSource.getRepository(EventImpactAnalysis);
  }
  ...
  async getFilteredEvents(query: CalendarQueryDto): Promise<EconomicEvent[]> {
    ...
    const queryBuilder =
      this.economicEventRepository.createQueryBuilder("event");
    ...
    if (importance && importance.length > 0) {
      queryBuilder.andWhere("event.importanceLevel IN (:...importance)", {
        importance,
      });
    }
    if (currencies && currencies.length > 0) {
      queryBuilder.andWhere("event.currencyAffected IN (:...currencies)", {
        currencies,
      });
    }
  ...
  async getUpcomingEvents(hours: number = 24): Promise<EconomicEvent[]> {
    ...
    return this.economicEventRepository
      .createQueryBuilder("event")
      .where(
        "CAST(event.date AS TEXT) || ' ' || CAST(event.time AS TEXT) >= :nowDateTime",
        { nowDateTime: now.toISOString().slice(0, 19).replace("T", " ") },
      ) // Format to 'YYYY-MM-DD HH:MM:SS'
      .andWhere(
        "CAST(event.date AS TEXT) || ' ' || CAST(event.time AS TEXT) <= :futureDateTime",
        {
  ...
  async createAlert(data: CreateAlertDto): Promise<UserEventAlert> {
    const eventExists = await this.economicEventRepository.findOneBy({
  ...
  async getUserAlerts(
    ...
    const queryBuilder =
      this.userEventAlertRepository.createQueryBuilder("alert");
  ...
  async analyzeEconomicEventImpact(
    eventId: string,
  ): Promise<EventImpactAnalysis> {
    ...
    let expectedImpactDirection: "Bullish" | "Bearish" | "Neutral" | "Mixed" =
      "Neutral";
      ...
      if (!isNaN(actualNum) && !isNaN(forecastNum)) {
        if (actualNum > forecastNum) {
          expectedImpactDirection =
            event.importanceLevel === "High" ? "Bullish" : "Neutral";
          ...
          expectedImpactDirection =
            event.importanceLevel === "High" ? "Bearish" : "Neutral";
          confidenceLevel = event.importanceLevel === "High" ? 8 : 5;
