...
class NFTCertificateService {
   ...
   */
  async verifyCertificateOwnership(certificateId: string, ownerPublicKey: string) {
    try {
      ...
      console.log(`Verifying certificate ${certificateId} ownership for ${ownerPublicKey}`);
      return true;
    } catch (error) {
  ...
  async transferCertificate(
    ...
    try {
      ...
      const currentOwnerKeypair = StellarService.createKeypairFromSecret(currentOwnerSecretKey);
