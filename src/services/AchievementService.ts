...
export class AchievementService extends BaseService<Achievement> {
  ...
  async updateProgress(
    ...
    if (userAchievement.progress >= 100) {
      ...
      this.sendNotification(
        ...
        `You've completed ${userAchievement.achievement.name}`,
      );
    }
  ...
  async findAll(
    ...
    try {
      ...
      console.log(
        `Fetched ${items.length} achievements for page ${page} with limit ${limit}`,
      );
      return { items, total };
    } catch (error) {
  ...
  async getUserProgress(
    userId: string,
  ): Promise<{ achievementId: string; progress: number }[]> {
  ...
  async getStatistics(): Promise<{
    ...
    const completed = await this.userAchievementRepo.count({
