...
export class AnalyticsService {
  ...
  async getUserStatistics(userId: string) {
    ...
    const averageCompletionPercentage = progress.reduce((sum, p) => sum + p.completionPercentage, 0) / (totalLessons || 1);
  ...
  async getLessonAnalytics(lessonId: string) {
    ...
    const completedAttempts = progress.filter(p => p.status === ProgressStatus.COMPLETED).length;
    const averageTimeSpent = progress.reduce((sum, p) => sum + p.timeSpent, 0) / (totalAttempts || 1);
    const averageCompletionPercentage = progress.reduce((sum, p) => sum + p.completionPercentage, 0) / (totalAttempts || 1);
  ...
  async getCourseCompletionRate(courseId: string) {
    ...
    const completionRates = Array.from(userProgress.entries()).map(([userId, completedLessonIds]) => ({
      userId,
    ...
    const averageCompletionRate = completionRates.reduce((sum, rate) => sum + rate.completionRate, 0) / (totalUsers || 1);
    const fullyCompletedUsers = completionRates.filter(rate => rate.completionRate === 100).length;
  ...
  async bulkUpdateProgress(updates: Array<{
    ...
    for (const update of updates) {
      try {
        ...
        if (update.completionPercentage) progress.completionPercentage = update.completionPercentage;
        if (update.metadata) progress.metadata = { ...progress.metadata, ...update.metadata };

        progress.lastInteractionAt = new Date();
        
        if (update.status === ProgressStatus.COMPLETED && !progress.completedAt) {
  ...
  async getBulkStatus(userIds: string[], lessonIds: string[]): Promise<BulkStatusResult> {
    const progress = await this.lessonProgressRepository.find({
      where: {
        userId: In(userIds),
